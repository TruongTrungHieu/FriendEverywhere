...
public class ConfirmFriendAsyncTask extends AsyncTask<String, Void, JSONObject> {

    private Callback callback;
    private Context context;

    public ConfirmFriendAsyncTask setCallback(Callback callback) {
        this.callback = callback;
        return this;
    }

    public ConfirmFriendAsyncTask(Context context) {
    ...
    protected JSONObject doInBackground(String... params) {
        ...
        listParams.add(new BasicNameValuePair("friend_status", params[2]));
        try {
            String result = NetworkSupport.sendRequest(Constants.URL_CONFIRM_FRIEND, listParams, NetworkSupport.RequestMethod.POST, 50000);
