 ...
 */
public class UpdateCurrentLocationAsyncTask extends AsyncTask<String, Void, JSONObject> {

    private Callback callback;
    private Context context;

    public UpdateCurrentLocationAsyncTask setCallback(Callback callback) {
        this.callback = callback;
        return this;
    }

    public UpdateCurrentLocationAsyncTask(Context context) {
    ...
    protected JSONObject doInBackground(String... params) {
        ...
        listParams.add(new BasicNameValuePair("latitude", params[1]));
        listParams.add(new BasicNameValuePair("longtitude", params[2]));
        try {
            String result = NetworkSupport.sendRequest(Constants.URL_UPDATE_CURRENT_LOCATION, listParams, NetworkSupport.RequestMethod.POST, 50000);
