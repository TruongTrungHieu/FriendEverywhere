...
public class CreatePasswordAsyncTask extends AsyncTask<String, Void, JSONObject> {

    private Callback callback;
    private Context context;

    public CreatePasswordAsyncTask setCallback(Callback callback) {
        this.callback = callback;
        return this;
    }

    public CreatePasswordAsyncTask(Context context) {
    ...
    protected JSONObject doInBackground(String... params) {
        ...
        listParams.add(new BasicNameValuePair("password", params[2]));
        try {
            String result = NetworkSupport.sendRequest(Constants.URL_CREATE_PASSWORD, listParams, NetworkSupport.RequestMethod.POST, 50000);
